...
const { api, ensureCookieMode, isPasskeyCancel } = useAppAuth()
...
async function loadMethods() {
  ...
  ensureCookieMode()
  try {
    const raw = await api<AuthMethodsResponse>('/auth/cookie/manage/authMethods', {
      auth: false
    })
...
async function postConfirm(body: ConfirmIdentityBody) {
  const response = await api<ConfirmIdentityResponse>('/auth/cookie/confirmIdentity', {
    method: 'POST',
    body,
    auth: false,
...
async function submitPasskey() {
  ...
  ensureCookieMode()
  try {
    const optionsJson = await api<string | object>('/auth/cookie/confirmIdentity/passkeyOptions', {
      method: 'POST',
      body: {},
      auth: false,
      csrfPath: '/auth/cookie/csrfToken'
