...
async function refreshAuthMethods() {
  try {
    authMethods.value = normalizeAuthMethods(await api('/auth/cookie/manage/authMethods', {
      auth: false
    }))
  } catch {
...
async function refreshPasskeys() {
  ...
  try {
    passkeyCount.value = readPasskeys(await api('/auth/passkey/passkeys/', {
      auth: false
    })).length
  } catch {
...
function startLink(scheme: string) {
  const returnUrl = `${window.location.origin}/app/security`
